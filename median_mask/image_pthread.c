...
//argumentos que serão mandados pras threads
struct args{
	RGB* imgout;
	RGB* imgin;
	int linha_ini;
...
void* calcula_mediana_linha(void* args) {

	ARGUMENTOS* a = (ARGUMENTOS*)args;

	int x, y, i, j, posvet;
	int borda  = a->tammask / 2;
	int tamvet = a->tammask * a->tammask;

	int altura  = a->altura;
	int largura = a->largura;
	...
	for(i = a->linha_ini; i < altura; i += a->numthread){
...
int main(int argc, char **argv ){
	...
	if (argc != 4) {
		printf("%s <nome do arquivo> <tamanho da mascara> <numero de threads>\n", argv[0]);
		exit(0);
	}
	...
	//======================= [alocar memória pra imagem] ============================

	//vetorzão da imagem original intacta
	RGB* imgin = malloc(cabecalho.altura * cabecalho.largura * sizeof(RGB));

	//mesma coisa pra imagem modificada
	RGB* imgout = malloc(cabecalho.altura * cabecalho.largura * sizeof(RGB));
	...
	//======================= [criar threads + cálculo e mediana] ============================

	//vetor com identificadores de cada thread
	pthread_t* tid = (pthread_t*)malloc(numthread* sizeof(pthread_t));

	//struct com todos parametros a serem passados pro thread
	ARGUMENTOS* a = (ARGUMENTOS*)malloc(numthread * sizeof(ARGUMENTOS));

	for (i = 0; i < numthread; i++) {

		a[i].imgin     = imgin;
		a[i].imgout    = imgout;
		a[i].altura    = cabecalho.altura;
		a[i].largura   = cabecalho.largura;
		a[i].tammask   = tammask;	
		a[i].numthread = numthread;

		//o unico que muda, a linha que inicia
		a[i].linha_ini = i;	

		//criar a thread
		pthread_create(&tid[i], NULL, calcula_mediana_linha, (void*)&a[i]);
	...
	//liberar imagem de entrada e structs das threads, não precisa mais
	free(imgin);
	free(tid);
	free(a);
